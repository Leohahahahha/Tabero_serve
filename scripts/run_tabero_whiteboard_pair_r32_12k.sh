...
usage() {
  echo 'Usage: CUDA_VISIBLE_DEVICES=0 bash scripts/run_tabero_whiteboard_pair_r32_12k.sh [--dry-run|--resume] PAIR_NAME'
...
gpu_count=${#gpu_ids[@]}
if (( gpu_count != 1 )); then
  echo 'This r32/12k launcher is configured for exactly one GPU.' >&2
  exit 2
fi
...
save_interval=4000
configs=(pi0_lora_tabero_whiteboard_next_state_force_tactile_r32_12k pi0_lora_tabero_whiteboard_sent_command_force_tactile_r32_12k)
...
build_train_command() {
  ...
  TRAIN_COMMAND=("$python_bin" -u scripts/train.py "$config" --exp-name="$run_name"
    ...
    --seed=42 --fsdp-devices=1 --batch-size="$train_batch_size" --num-workers=4
    --num-train-steps="$target_steps" --lr-schedule.warmup-steps=500
    --lr-schedule.decay-steps="$target_steps" --lr-schedule.peak-lr=1e-5
    --lr-schedule.decay-lr=1e-6 --eval-interval=1000 --eval-num-batches="$eval_num_batches"
...
if [[ $mode == dry-run ]]; then
  echo "Pair: $pair_name; GPUs: $CUDA_VISIBLE_DEVICES; global batch: $train_batch_size; target: 7D action + 6D wrist wrench; updates per model: $target_steps"
  for i in 0 1; do
    config=${configs[$i]}
