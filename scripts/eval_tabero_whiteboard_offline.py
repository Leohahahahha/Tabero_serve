"""Offline whiteboard action+wrench evaluation on held-out recordings; never connects to a robot."""
...
def parse_args(argv=None):
    ...
    parser.add_argument("--config", required=True, help="Matching whiteboard action+wrench training config")
...
def validate_model_contract(config):
    """Accept the rank-16/rank-32/full-parameter whiteboard action+wrench contract."""
    ...
    if (
        ...
        or config.policy_metadata.get("prediction_layout") != "7d_action_then_6d_wrist_wrench"
        or not config.policy_metadata.get("predicts_wrench")
        or config.policy_metadata.get("output_action_dim") != 7
        or config.policy_metadata.get("wrist_wrench_dim") != 6
        or model.supervised_action_dim is not None
        or model.effective_action_dim != 13
        or model.action_dim != 32
        or model.action_horizon != 50
    ):
        raise ValueError("Use a whiteboard absolute 7D action + 6D wrist-wrench config")
    if model.tactile_type is TactileType.NO or model.tactile_streams != ("tactile_prefix",):
        raise ValueError("Whiteboard models require the tactile marker-motion prefix")
    rank = model.tactile_prefix_lora_rank
    if rank not in (0, 16, 32) or config.policy_metadata.get("tactile_lora_rank") != rank:
        raise ValueError("Unsupported or inconsistent tactile adaptation rank")
    return rank
...
def main(argv=None):
    ...
    tactile_lora_rank = validate_model_contract(config)
    ...
    required_stats = {"state", "actions", "tactile_prefix"}
    ...
    manifest = {
        "status": "initializing",
        "config": args.config,
        "input_modality": "rgb_state_touch",
        "prediction_layout": "7d_action_then_6d_wrist_wrench",
        "tactile_lora_rank": tactile_lora_rank,
        "action_source_mode": config.policy_metadata.get("action_source_mode"),
        "action_label_source": config.policy_metadata.get("action_label_source"),
        "optimization_method": config.policy_metadata.get("optimization_method", "lora_finetuning"),
        ...
        if args.check_only:
            ...
            for episode, length in lengths.items():
                for frame in sorted({0, length - 1}):
                    ...
                    offline.policy_observation(sample, use_tactile=True)
                    ...
                    wrist_wrench = np.asarray(sample["wrist_wrench"])
                    if wrist_wrench.shape != (horizon, 6) or not np.isfinite(wrist_wrench).all():
                        raise ValueError("Invalid raw wrist-wrench targets")
