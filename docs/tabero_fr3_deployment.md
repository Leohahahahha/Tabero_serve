...
- `prompt`：必须与训练集 `meta/tasks.jsonl` 中的 task 文本一致；当前任务是将黑色圆形部件对准灰色圆形部件的孔并插入完成装配；
...
正式预测逐步写入 JSONL。客户端对每份当前观测同步等待一次推理；每个结果写一条
`inference_chunk`，包含唯一`chunk_id`、推理延迟、模型看到的状态和完整`50×7`动作块。
完整 chunk 仅用于诊断，控制固定使用`action[0]`。每次控制写一条`control_tick`，包含原始
`action[0]`、边界投影后的`bounded_prediction`、逐周期限速后的`limited_action`、HTTP payload，
以及`limits_applied`和`saturated`。`distances`给出原始/投影/最终动作相对观测和当前状态的
位置、SO(3)最短旋转角及单指开度差，也记录相邻两次`action[0]`的变化。
...
有限的预测越过工作空间、最大目标位移、最大目标旋转或夹爪范围时，会被投影到边界；逐周期
位置、旋转和夹爪速度也会被限幅。日志会保留原始预测和全部触发项，不会静默隐藏模型异常。
实测状态越界、跟踪误差、NaN/Inf、观测断流、模型通信错误和推理过期仍会终止程序。

检查日志里的坐标、姿态和夹爪趋势与实际场景相符，并统计`saturated`比例；若大部分周期都触发
限幅，模型仍不适合执行，即使客户端能够构造一个边界内命令。
当前模型此前的离线结果存在较大姿态误差、夹爪超范围及 episode 起始标签问题；
部署接口写好并不意味着该 checkpoint 已具备真机任务成功率。
若 shadow 持续饱和，先处理数据/模型误差，不要直接放宽阈值来让它运动。
...
多个条件同时触发时，最终目标满足所有约束，因此不一定停在某一个单独约束的几何边界上。
