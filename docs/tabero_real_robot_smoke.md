...
按数据集的10 Hz频率，50个预测动作对应5秒。同步部署默认每份观测只使用`action[0]`；可用
`--actions-per-inference 2`依次执行`action[0:2]`，两步之间保持100 ms，并在每一步重新读取实测
状态、检查跟踪误差和持续使能。第二步若已超过350 ms时效门限会被丢弃并重新推理。当前约
200 ms推理延迟下先用K=2做shadow，不执行K=3或完整50步开环。实际动作频率和截断率需从
`control_tick`与`chunk_truncated`日志测量。
